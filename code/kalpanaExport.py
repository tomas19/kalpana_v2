...
def runExtractContours(ncObj, var, levels, conType, epsg, subDom, npro):
    ...
    if timeVar == 0:
        ...
        if npro == 1:
            ...
            if conType == 'polyline':
                listGdf = p.starmap(contours2gpd_mp, fArgs)
                p.close()
                p.join()
                
            elif conType == 'polygon':
                listGdf = p.starmap(filledContours2gpd_mp, fArgs)
                p.close()
                p.join()
            else:
                print('only "polyline" and "polygon" types are supported!')
...
def nc2shp(ncFile, var, levels, conType, epsg, pathOut, npro=1, subDomain=None, dateSubset=None):
    ...
    # if subDomain == None:
    # runExtractContours(ncObj, var, levels, conType, epsg, subDom=None, npro=1):
